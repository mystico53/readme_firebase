...
export const cleanText = functions.https.onRequest((request, response) => {
  cors(request, response, async () => {
    try {
      console.log("Function triggered with request:", JSON.stringify(request.body));
      // const instructions = "The following text might be messy, I want to read it to someone, extract the article text verbatim. The goal is to have the full article text without any clutter. Do not include any captions of photos.";
      const instructions = "summmarize the three key points of the text";
